...
class _AssessmentScreenState extends State<AssessmentScreen> {
  ...
  void initState() {
    ...
    if (assessmentQuestions.isNotEmpty) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          setState(() {
            _errorMessage = "خطأ: لم يتم تحميل أسئلة التقييم.";
            _isLoading = false;
          });
        }
      });
      ...
      if (mounted) {
        setState(() {
          _chatMessages.add(ChattMessage(
            text:
                "أهلاً بكِ. سأطرح عليكِ بعض الأسئلة حول ملاحظاتك لطفلك. يرجى الإجابة بنص حر أو استخدام الأزرار المقترحة.",
            isUserMessage: false,
          ));
        });
        _scrollToBottom();
  ...
  void _askQuestionAtIndex(int index) {
    if (index < assessmentQuestions.length && mounted) {
      ...
      Future.delayed(const Duration(milliseconds: 500), () {
        ...
        index >= assessmentQuestions.length &&
        !_isSessionComplete) {
      ...
      setState(() {
        _errorMessage = "لا يمكن عرض السؤال التالي.";
      });
    }
  }
    ...
    if (explanation == null || explanation.isEmpty) {
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text("لا يوجد شرح إضافي لهذا السؤال."),
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  // --- العنوان ---
                  Row(
                    // الصف يبقى كما هو لكن المحتوى داخله سيتبع Directionality
                    children: [
  ...
  Future<void> _handleAnswer(String answerText) async {
    ...
    if (trimmedAnswer.isEmpty ||
        _isLoading ||
        _isSessionComplete ||
        _currentIndex >= assessmentQuestions.length) return; // إضافة تحقق إضافي
    ...
    const List<String> explanationRequests = [
      "مش فاهمة",
      "مش فاهمه",
      "يعنى ايه",
      "يعني ايه",
    ...
    bool needsExplanation =
        explanationRequests.any((request) => lowerCaseAnswer.contains(request));
    ...
    setState(() {
      ...
      _errorMessage = null;
      _chatMessages
          .add(ChattMessage(text: rawAnswerForDisplay, isUserMessage: true));
      _answerController.clear();
    });
    _scrollToBottom();
    ...
    if (validKeywords.contains(trimmedAnswer)) {
      ...
      classifiedAnswer =
          await ApiService.classifyAnswerWithModel(trimmedAnswer);
    }

    if (!mounted) return;

    // 4. معالجة نتيجة التصنيف
    if (classifiedAnswer == null) {
      // يشمل فشل المودل أو عدم وجود الكلمة المفتاحية
      setState(() {
        _errorMessage =
            "لم يتم تحليل الإجابة. حاول مرة أخرى أو استخدم الاقتراحات.";
        _isLoading = false;
        if (_chatMessages.isNotEmpty && _chatMessages.last.isUserMessage) {
    ...
    // 6. إرسال للباك اند
    bool backendSuccess = await ApiService.submitAssessmentAnswer(
        currentQuestion.id, classifiedAnswer, widget.jwtToken);
    ...
    if (backendSuccess) {
      ...
      setState(() {
        _errorMessage = "خطأ في إرسال الإجابة.";
        _isLoading = false;
        if (_chatMessages.isNotEmpty && _chatMessages.last.isUserMessage) {
          _chatMessages.removeLast();
        }
    ...
    if (mounted && _isLoading && !backendSuccess) {
      setState(() {
        _isLoading = false;
      });
    }
  ...
  Future<void> _moveToNextQuestionOrComplete() async {
    ...
    while (nextIndex < assessmentQuestions.length) {
      ...
      if (_skipActive && nextQuestion.emotion == _emotionToSkip) {
        ...
        bool skipBackendSuccess = await ApiService.submitAssessmentAnswer(
            nextQuestion.id, "لا", widget.jwtToken);
        // --- (التعليق على تخزين نتيجة التخطي التفصيلية) ---
        // _submissionResults.add(SubmissionResult( ... ));
        if (!mounted) return;
        if (!skipBackendSuccess) {
          errorDuringSkip = true;
          if (mounted)
            setState(() {
              _errorMessage = "خطأ بتحديث سؤال متخطى (${nextQuestion.id}).";
            });
          break;
        }
  ...
  void _completeAndGoHome() {
    if (mounted && !_isSessionComplete) {
      // تحقق إضافي لمنع الاستدعاء المتعدد
      setState(() {
...
// --- التمرير لأسفل ---
      ...
      if (mounted && _scrollController.hasClients) {
        ...
        _scrollController.animateTo(
          _scrollController.position
              .maxScrollExtent, // الانتقال إلى أقصى نقطة يمكن التمرير إليها (الأسفل)
          duration:
              const Duration(milliseconds: 300), // مدة الأنيميشن (يمكن تعديلها)
        ...
        children: [
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final borderRadius = message.isUserMessage
          ...
          if (!_isSessionComplete)
                      ...
                      children: [
                          ...
                          child: Container(
                            ...
                            child: TextField(
                              ...
                              onSubmitted: _isLoading
                                  ? null
                                  : (value) => _handleAnswer(value.trim()),
                            ),
                          ),
                        ),
                        const SizedBox(width: 8),
                        FloatingActionButton(
                          ...
                          onPressed: _isLoading
                              ? null
                              : () =>
                                  _handleAnswer(_answerController.text.trim()),
                          child: _isLoading
                              ? const SizedBox(
                                  width: 20,
                                  height: 20,
            ...
            Container(
                ...
                child: const Center(
                    child: Text("اكتمل التقييم، شكراً لكِ.",
  ...
  Widget _buildSuggestionButton(String text) {
    return OutlinedButton(
      onPressed: _isLoading
          ? null
          : () => _handleAnswer(text), // استدعاء المعالج مباشرة
      child: Text(text),
      style: OutlinedButton.styleFrom(
          foregroundColor: Theme.of(context).primaryColor,
          side:
              BorderSide(color: Theme.of(context).primaryColor.withAlpha(100)),
